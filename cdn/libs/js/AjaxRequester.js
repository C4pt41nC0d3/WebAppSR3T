    ...
    var fieldsobject = {}, 
      ...
      $.each(fieldsarray, function(){
        if(fieldsobject[this.name] !== undefined){
          ...
          fieldsobject[this.name] = this.value || '';
      });
     return fieldsobject; 
  }
};
...
AjaxRequester.prototype.http_status_codes = {
  400: function(){
	...
	403: function(){
		ajaxrequester.showAppriseAlert(
			"cdn/img/site/hacker.jpg", 
			"danger", "<b>Error!</b> Estás intentando inyectar código SQL, piensa más la próxima vez, si vas a vulnerar este sistema debes de ser más inteligente, saludos noobster.<br><b>El poder del hacking web nos reespalda, captaincode.</b>", {
				buttons: {
					confirm:  {
						text: "Cerrar [Soy un noobster]",
						className: "red", 
						action: function(e){
							Apprise("close");
						}
...
/*
	...
	url [string]: url of http request.
	_method [string]: http method.
	bcallback [function]: callback before the execution of http request.
	_cache [boolean]: true stores the requested pages by the browser.
	ccallback [function]: callback that is executed after and before of the request.
	ecallback [function]: callback that is executed, just if the http request has presented one error.
	httpheaders [object]: send the request with http headers.
	scallback [function]: callback executed just if the http request was complete with success.
	data [object]: is the data object, for send the http request.
*/

AjaxRequester.prototype.SendHTTPRequest = function(_url, _method, _data, bcallback, _cache, ccallback, ecallback, httpheaders, scallback){
	if(typeof _url === "string" 
		&& typeof _method === "string" 
		&& typeof _data === "object"
		&& typeof _cache === "boolean"
		&& (_method === "post" 
		|| _method === "get"))
		  $.ajax({
		   url: this.host+_url,
			 method: _method,
			 data: _data,
			 beforeSend: bcallback,
			 cache: _cache,
			 complete: ccallback,
			 dataType: "json",
			 error: ecallback,
			 headers: httpheaders,
			 statusCode: this.http_status_codes,
       success: scallback,
       traditional: true
		  });
...
AjaxRequester.prototype.showAppriseAlert = function(img, type, message, appriseobject){
	if(typeof img === "string" 
		&& typeof type === "string" 
		&& typeof message === "string" 
		&& typeof appriseobject === "object" 
		&& (type === "success" 
			|| type === "info" 
			|| type === "warning" 
			|| type === "danger")
		)Apprise("<div class='row apprise-alerts'><div class='col-md-2'><img src='"+img+"' class='img-circle'></div><div class='col-md-10'><div class='alert alert-"+type+"' role='alert'>"+message+"</div></div></div>", appriseobject);
}
...
Register.prototype.push = function(modulename, controller){
	if(typeof modulename === "string" 
		&& typeof controller === "function"
		&& typeof this.modules[modulename] === "undefined")
...
Iterator.prototype.init = function(object, handler){
	if(typeof object === "object" && typeof handler === "function"){
		...
		var i = 0;
		for(var modulename in object){this.object[i] = object[modulename];i++;}
	}
}
